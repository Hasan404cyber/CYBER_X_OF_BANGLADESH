...
'''
</> OPEN SOURCED BY - CYBER_X_OF_BANGLADESH
...
def GetInfoProfile():
        with open('/sdcard/AUTO-LMNx9/SUCCESS-OK-ID.txt', 'r') as file:
...
logo=("""
    </> OPEN SOURCED BY - CYBER X OF BANGLADESH
    </> TELEGRAM - https://t.me/CYBER_X_OF_BANGLADESH
""")
ll=str([hari,tanggal])
kk=str([xy,co])
hx=(f""" </> OPEN SOURCED BY - CYBER_X_OF_BANGLADESH """)
def banner():
    os.system("clear")
...
def progres(current,num_accounts,delay):
        for sleep in range(int(delay), 0, -1):
            print(f'[bold magenta1]LMNx9 [bold white][[bold cyan]{current}[bold white]/[bold red]{num_accounts}[bold white]] SUCCESS-:[bold green1]{Ok}\r',end='\r')
            time.sleep(1)
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            dn()
            open("/sdcard/AUTO-LMNx9/SUCCESS-OK-ID.txt","a").write(uid+f"|{passw}|"+cookie+"\n")
            Ok+=1
    except Exception as e:
        pass
